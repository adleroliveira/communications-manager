import { CommunicationsManager, ICommunicationsManagerConfig } from '../src/CommunicationsManager';
...
describe('CommunicationsManager', () => {
  ...
  test('constructor initializes all managers correctly', () => {
    ...
    expect(HeartbeatManager).toHaveBeenCalledWith(expect.any(Object), config.heartbeatInterval);
  });

  test('constructor throws error when URL is not provided', () => {
    const invalidConfig = { ...config, url: '' };
    expect(() => new CommunicationsManager(invalidConfig)).toThrow('URL is required in the configuration');
  });
  ...
  test('setAuthToken calls AuthenticationManager.setAuthToken', () => {
    ...
    expect(AuthenticationManager.prototype.setAuthToken).toHaveBeenCalledWith(newToken);
  });
  ...
  test('subscribe calls SubscriptionManager.subscribe', async () => {
    ...
    expect(SubscriptionManager.prototype.subscribe).toHaveBeenCalledWith(channel);
  });
  ...
  test('onOpen registers open event callback', () => {
    ...
    expect(WebSocketManager.prototype.on).toHaveBeenCalledWith('open', callback);
  });
  ...
  test('onClose registers close event callback', () => {
    ...
    expect(WebSocketManager.prototype.on).toHaveBeenCalledWith('close', callback);
  });
  ...
  test('onError registers error event callback', () => {
    ...
    expect(WebSocketManager.prototype.on).toHaveBeenCalledWith('error', callback);
  });
  ...
  test('onMessage registers message event callback', () => {
    ...
    expect(WebSocketManager.prototype.on).toHaveBeenCalledWith('message', callback);
  });
  ...
  test('onRequest registers request callback', () => {
    ...
    expect(RequestManager.prototype.on).toHaveBeenCalledWith(requestType, callback);
  });
  ...
  test('handleError logs the error and unsubscribes from all channels', () => {
    ...
    expect(mockLogger.error).toHaveBeenCalledWith('WebSocket error:', { error: testError });
  ...
  test('handleClose logs the event, stops the heartbeat, and unsubscribes from all channels', () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith('WebSocket connection closed', { event: testCloseEvent });
